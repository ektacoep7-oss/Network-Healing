...
class FitnessWeights:
    ...
    - Increased fault_history from 0.15 → 0.20 (prevent greedy optimization)
    - Adjusted others to maintain total = 1.0
    """

    proximity: float = 0.20  # w3: latency weight (reduced from 0.25, too aggressive)
    communication_cost: float = 0.15  # w2: network penalty
    residual_energy: float = 0.25  # w1: headroom/capacity (keep high)
    coverage: float = 0.20  # w4: serveability
    fault_history: float = 0.20  # w5: fault penalty (INCREASED for reliability)
