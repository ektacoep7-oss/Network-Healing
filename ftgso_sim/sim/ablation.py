...
def main():
    parser = argparse.ArgumentParser(description="SC-FTGSO ablation study.")
    parser.add_argument("--n-instances",  type=int,   default=80)
    parser.add_argument("--n-groups",     type=int,   default=5)
    parser.add_argument("--n-steps",      type=int,   default=500)
    parser.add_argument("--request-rate", type=float, default=20.0)
    ...
    # Ablation variants: add one component at a time
    ablation_configs = {
        "baseline (RR)":            dict(policy="round_robin", with_healing=False),
        "+ GSO routing":            dict(policy="gso",         with_healing=False),
        "+ healing L1":             dict(policy="gso",         with_healing=True),
        "+ adaptive weights (★)":  dict(policy="gso",         with_healing=True, adaptive_weights=True),
        "+ gossip (★)":            dict(policy="gso",         with_healing=True, use_gossip=True),
        "full SC-FTGSO (★)":      dict(policy="gso",         with_healing=True,
                                        adaptive_weights=True, use_gossip=True),
    }

    results = {}
    for name, kwargs in ablation_configs.items():
        m, _ = _simulate_one_policy(cfg, weights=weights, init_instances=instances, **kwargs)
        results[name] = m

    # Print ablation table
    print(f"\n{'Variant':<28} {'PDR':>6} {'PLR':>6} {'E2E ms':>8} {'MTTH':>6}")
    print("-" * 58)
    for name, m in results.items():
        print(f"  {name:<26} {m['pdr']:>6.3f} {m['plr']:>6.3f} "
              f"{m['e2e_ms']:>8.1f} {m['mtth']:>6.1f}")

    # Save CSV
    out_csv = output_dir / "ablation_summary.csv"
    with out_csv.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["variant", "pdr", "plr", "e2e_ms", "mtth"])
        for name, m in results.items():
            writer.writerow([name, f"{m['pdr']:.6f}", f"{m['plr']:.6f}",
                              f"{m['e2e_ms']:.6f}", f"{m['mtth']:.2f}"])

    # Plots
    for metric, title in [("pdr", "Ablation: PDR (higher = better)"),
                           ("plr", "Ablation: PLR (lower = better)"),
                           ("e2e_ms", "Ablation: E2E ms (lower = better)"),
                           ("mtth",   "Ablation: MTTH steps (lower = faster healing)")]:
        labels = list(results.keys())
        ...
        fig.tight_layout()
        fig.savefig(output_dir / f"ablation_{metric}.png", dpi=160)
        plt.close(fig)
